...
export default function Grados() {
  ...
  async function guardarGrados() {
    ...
    const combinacionesExistentes = combinaciones.filter((comb) => {
      return gradosFiltrados.some(
        (g) => g.nombre === comb.nombre && g.paralelo === comb.paralelo,
      );
    });

    if (combinacionesExistentes.length > 0) {
      const mensajes = combinacionesExistentes
        .map((c) => `${c.nombre} - ${c.paralelo}`)
        .join("\n");
      mostrarToast(
        "warning",
        "Grados ya existentes",
        `Los siguientes grados ya existen:\n${mensajes}`,
        6000,
      );
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="p-5">
            ...
            <div className="flex gap-2 pt-3 border-t border-slate-200">
              <button
                type="submit"
                disabled={
                  selectedNiveles.length === 0 || selectedParalelos.length === 0
                }
