...
export default function Layout({
  ...
  // ✅ Drawer móvil (se abre con el botón ☰ en pantallas pequeñas)
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 to-slate-100 flex">
      ...
      <aside
        className={`hidden md:flex md:flex-col shrink-0 bg-white border-r border-slate-200 shadow-sm z-30 transition-all duration-200
          md:sticky md:top-0 md:h-screen
          ${railOpen ? 'md:w-56' : 'md:w-16'}`}
      >
        {/* Logo + toggle */}
        ...
        <div className={`border-t border-slate-100 p-2 relative ${railOpen ? '' : 'flex justify-center'}`}>
          ...
          {showDropdown && (
            <>
              ...
              {/* Anclado arriba del botón de usuario (se abre hacia arriba para no salirse de la pantalla) */}
      ...
      <header className="md:hidden sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm">
        ...
        {mobileMenuOpen && (
          <div className="border-t border-slate-100 bg-white px-2 py-2 max-h-[70vh] overflow-y-auto">
            {navItems.map((item) => {
              const activo = location.pathname === item.to;
              const Icon = item.icon;
              return (
