...
export default function Estudiantes() {
  ...
  // ✅ Tiempo real con onSnapshot (spinner derivado, sin setState síncrono)
  useEffect(() => {
    if (!ready) return;
...
<body>
  ...
  <div class="meta">
    <div class="meta-box"><div class="lbl">Grado</div><div class="val">${gradoActual.nombre} — Paralelo "${gradoActual.paralelo}"</div></div>
    <div class="meta-box"><div class="lbl">${esTutor ? "Tutor(a)" : "Responsable"}</div><div class="val">${responsable}</div></div>
    <div class="meta-box"><div class="lbl">Fecha de emisión</div><div class="val">${fechaGen}</div></div>
  </div>
  <table>
    <thead><tr><th class="num">N°</th><th>Apellidos</th><th>Nombres</th><th class="cedula">Cédula / Código</th><th class="estado">Estado</th></tr></thead>
    <tbody>${filas}</tbody>
  </table>
  <div class="summary">
  ...
  <div class="signatures">
    <div class="sig"><div class="sig-line"><div class="rol">${esTutor ? "Tutor(a) del Grado" : "Docente"}</div><div>${responsable}</div></div></div>
    <div class="sig"><div class="sig-line"><div class="rol">Vicerrector(a)</div><div>&nbsp;</div></div></div>
  </div>
  ...
  <script>window.onload = function () { setTimeout(function () { window.print(); }, 400); };</script>
    ...
    if (!printWindow) {
      mostrarToast("warning", "Ventana bloqueada", "Permite las ventanas emergentes para poder imprimir la nómina.");
      return;
    }
  ...
  const handleExportExcel = () => {
    if (estudiantesAMostrar.length === 0) {
      mostrarToast("warning", "Sin datos", "No hay estudiantes para exportar.");
      return;
    }
    try {
      ...
      const gradoSeleccionado = gradosFiltrados.find((g) => g.id === gradoEfectivoId);
      ...
      if (gradoSeleccionado) {
        nombreArchivo = `Nomina_${gradoSeleccionado.nombre.replace(/ /g, "_")}_${gradoSeleccionado.paralelo.replace(/ /g, "_")}_${fechaStr}.xlsx`;
      } else {
        nombreArchivo = `Nomina_Todos_${fechaStr}.xlsx`;
      }
      XLSX.writeFile(wb, nombreArchivo);
      mostrarToast("success", "Excel descargado", `Se exportaron ${listaOrdenada.length} estudiante${listaOrdenada.length !== 1 ? "s" : ""}.`);
    } catch (error) {
      console.error("Error exportando a Excel:", error);
      mostrarToast("error", "Error al exportar", "No se pudo descargar el archivo Excel.");
    }
  };
  ...
  async function guardarEstudiantesMasivos() {
    ...
    students.forEach((s) => {
      if (cedulasVistas.has(s.cedula)) duplicadosInternos.push(s.cedula);
      else cedulasVistas.add(s.cedula);
    });
    if (duplicadosInternos.length > 0) {
      setValidationErrors([`Existen cédulas/códigos duplicados en la lista: ${[...new Set(duplicadosInternos)].join(", ")}`]);
      return;
    }
    setIsSavingMassive(true);
    try {
      const mapaExistentes = new Map(
        estudiantes.filter((e) => e.gradoId === gradoEfectivoId).map((e) => [e.cedula, e]),
      );
      ...
      students.forEach((s) => {
        ...
        if (!existente) aCrear.push(s);
        else if (existente.activo) duplicadosActivos.push(s.cedula);
        else aReactivar.push({ ...s, id: existente.id });
      });
      if (duplicadosActivos.length > 0) {
        setValidationErrors([
          `Las siguientes cédulas/códigos ya están ACTIVOS en este grado: ${[...new Set(duplicadosActivos)].join(", ")}.`,
      ...
      mostrarToast("success", "Registro completado", `Procesados ${students.length} estudiante(s): ${detalle.join(", ")}.`, 5000);
      setMassiveData("");
      setValidationErrors([]);
  ...
  const validarEstudiante = (
    ...
    const errors: string[] = [];
    if (!cedula || cedula.trim() === "") errors.push("La cédula/código es obligatorio");
    else if (cedula.trim().length < 1 || cedula.trim().length > 10)
      errors.push(`La cédula/código debe tener entre 1 y 10 caracteres`);
    if (!apellidos || apellidos.trim() === "") errors.push("Los apellidos son obligatorios");
    if (!nombres || nombres.trim() === "") errors.push("Los nombres son obligatorios");
    if (cedula) {
      const duplicado = estudiantes.find((e) => e.cedula === cedula.trim() && e.id !== excludeId);
      if (duplicado) {
        if (duplicado.activo)
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "estudiantes", editingId), {
          ...
          conAdaptacion: formData.conAdaptacion,
          updatedAt: serverTimestamp(),
        });
      }
      resetForm();
  ...
  const handleEdit = (estudiante: Estudiante) => {
    ...
    setFormData({
      ...
      conAdaptacion: estudiante.conAdaptacion || false,
    });
  ...
  async function handleDelete(id: string) {
    ...
    const confirmado = await confirmar(
      ...
      { confirmText: "Sí, desactivar", cancelText: "Cancelar", confirmColor: "bg-orange-600 hover:bg-orange-700", icon: FaTimes },
    );
    if (!confirmado) return;
    try {
      await updateDoc(doc(db, "estudiantes", id), { activo: false, updatedAt: serverTimestamp() });
  ...
  async function handleToggleActivo(id: string, estadoActual: boolean) {
    ...
    try {
      await updateDoc(doc(db, "estudiantes", id), { activo: !estadoActual, updatedAt: serverTimestamp() });
      mostrarToast("success", estadoActual ? "Estudiante desactivado" : "Estudiante reactivado", `"${estudiante.apellidos} ${estudiante.nombres}" ${estadoActual ? "fue desactivado" : "fue reactivado correctamente"}.`);
    } catch (error) {
  ...
  const estudiantesAMostrar =
    userData?.role === "docente" && !gradoEfectivoId ? [] : estudiantesFiltrados;
            ...
            {esAdmin && (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border-2 ${
                  gradoEfectivoId === null ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-700 border-slate-200 hover:border-blue-400"
                }`}
              >
                Todos los grados
              </button>
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border-2 flex items-center gap-2 ${
                    gradoEfectivoId === grado.id ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-700 border-slate-200 hover:border-blue-400"
                  }`}
                >
                  <span>{grado.nombre} - {grado.paralelo}</span>
                  {esTutor && <span className="text-xs bg-yellow-400 text-yellow-900 px-1.5 py-0.5 rounded">Tutor</span>}
                </button>
              );
            })}
          </div>
      ...
      {editingId && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 mb-6 overflow-hidden">
          <div className="bg-linear-to-r from-blue-600 to-blue-700 px-5 py-3 flex items-center justify-between">
            ...
            <button type="button" onClick={() => { resetForm(); setEditingId(null); }} className="text-white text-sm hover:bg-white/20 px-3 py-1 rounded transition">
              Cancelar
            </button>
          </div>
          <form onSubmit={handleSubmit} className="p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input type="text" value={formData.cedula} onChange={(e) => setFormData({ ...formData, cedula: e.target.value })} placeholder="Ej: 1712345678 o A123" maxLength={10} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" required />
              ...
              <div>
                ...
                <input type="text" value={formData.apellidos} onChange={(e) => setFormData({ ...formData, apellidos: e.target.value })} placeholder="Ej: Pérez García" className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" required />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">Nombres *</label>
                <input type="text" value={formData.nombres} onChange={(e) => setFormData({ ...formData, nombres: e.target.value })} placeholder="Ej: Juan Carlos" className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" required />
              </div>
              <div className="flex items-center">
                <input type="checkbox" id="activo" checked={formData.activo} onChange={(e) => setFormData({ ...formData, activo: e.target.checked })} className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500" />
                <label htmlFor="activo" className="ml-2 text-sm text-slate-700">Estudiante activo</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" id="conAdaptacion" checked={formData.conAdaptacion} onChange={(e) => setFormData({ ...formData, conAdaptacion: e.target.checked })} className="w-4 h-4 text-purple-600 border-slate-300 rounded focus:ring-purple-500" />
                <label htmlFor="conAdaptacion" className="ml-2 text-sm text-slate-700">Con adaptación curricular (NECE)</label>
              </div>
            </div>
      ...
      {showMassiveForm && puedeRegistrar && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 mb-6 overflow-hidden">
          <div className="bg-linear-to-r from-green-600 to-green-700 px-5 py-3 flex items-center justify-between">
            <h3 className="text-white font-semibold text-base flex items-center gap-2"><FaUpload /> Registro Masivo de Estudiantes</h3>
            <button type="button" onClick={() => { setShowMassiveForm(false); setMassiveData(""); setValidationErrors([]); }} className="text-white text-sm hover:bg-white/20 px-3 py-1 rounded transition">Cerrar</button>
          </div>
          <div className="p-5">
            {validationErrors.length > 0 && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <ul className="text-red-700 text-sm space-y-1">{validationErrors.map((error, idx) => (<li key={idx}>• {error}</li>))}</ul>
                  </div>
                </div>
              </div>
            )}
            <div className="mb-4">
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">Lista de Estudiantes (Uno por línea)</label>
              <textarea value={massiveData} onChange={(e) => { setMassiveData(e.target.value); setValidationErrors([]); }} placeholder={"Ejemplo (cualquiera de los dos formatos funciona):\n\n1712345678 PEREZ GARCIA JUAN CARLOS\n1723456789 LOPEZ MARTINEZ MARIA ELENA\n\nO con coma:\n1712345678, PEREZ GARCIA JUAN CARLOS\n1723456789, LOPEZ MARTINEZ MARIA ELENA"} rows={10} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all" />
              <div className="text-xs text-slate-500 mt-2 space-y-1">
                <p className="flex items-start gap-1"><FaInfoCircle className="mt-0.5 shrink-0" /><span><strong>Formatos aceptados:</strong> Puedes pegar directamente desde Word, Excel o bloc de notas.</span></p>
            ...
            <div className="flex gap-2 pt-3 border-t border-slate-200">
              <button onClick={guardarEstudiantesMasivos} disabled={isSavingMassive || !massiveData.trim()} className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-6 py-2.5 rounded-lg transition-all text-sm font-semibold">
                {isSavingMassive ? (<><FaSpinner className="text-xs animate-spin" /> Validando y guardando...</>) : (<><FaCheck className="text-xs" /> Guardar Estudiantes</>)}
              </button>
              <button type="button" onClick={() => { setShowMassiveForm(false); setMassiveData(""); setValidationErrors([]); }} className="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2.5 rounded-lg transition-all text-sm font-medium">
                <FaTimes className="text-xs" /> Cancelar
              </button>
            </div>
          </div>
            ...
            <>
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 mb-6 p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="relative w-full sm:w-96">
                  ...
                  <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar por apellidos, nombres, cédula..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" />
                </div>
                <div className="flex gap-2 flex-wrap">
                  {(esAdmin || (userData?.role === "docente" && gradoEfectivoId)) && estudiantes.length > 0 && (
                    <>
                      {estudiantesAMostrar.length > 0 && (
                        <button onClick={handleExportExcel} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-sm">
                          <FaFileExcel /> Descargar Excel
                        </button>
                      )}
                      <button onClick={handlePrintNomina} className="inline-flex items-center gap-2 bg-slate-700 hover:bg-slate-800 text-white px-4 py-2 rounded-lg transition-all text-sm font-medium shadow-sm" title="Imprimir nómina completa (activos e inactivos)">
                  ...
                  {puedeRegistrar && (
                    <button onClick={() => { setShowMassiveForm(true); setValidationErrors([]); }} className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition-all text-sm font-medium shadow-sm">
                  ...
                  <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200 hidden md:table-header-group">
                      <tr>
                        ...
                        {esAdmin && <th className="px-5 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Grado</th>}
                        {puedeRegistrar && (
                          <>
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {cargandoLista ? (
                        ...
                        <tr>
                          <td colSpan={4} className="px-5 py-16 text-center">
                            <div className="flex flex-col items-center">
                              <div className="bg-slate-100 rounded-full p-4 mb-3"><FaUsers className="text-3xl text-slate-400" /></div>
                              <p className="text-slate-600 font-medium mb-1">{searchTerm ? "No se encontraron estudiantes" : "No hay estudiantes registrados"}</p>
                              {!searchTerm && puedeRegistrar && <p className="text-slate-500 text-sm">Utilice el botón "Registrar Estudiantes" para agregar alumnos a este grado.</p>}
                            </div>
                          </td>
                        </tr>
                      ) : (
                            ...
                            <tr key={est.id} className={`block md:table-row border-b md:border-b-0 border-slate-100 last:border-b-0 hover:bg-slate-50 transition-colors ${!est.activo ? "bg-red-50/40" : ""}`}>
                              <td className={`px-5 py-4 block md:table-cell ${!est.activo ? "opacity-60" : ""}`}>
                                <div className="flex flex-col">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    {est.conAdaptacion && (
                                      <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-100 border border-purple-300 text-purple-700 rounded text-[10px] font-bold">
                                        Adaptación
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  {puedeEditar && (
                                    <div className="mt-3 flex flex-col gap-2 md:hidden">
                                      <button onClick={() => handleToggleActivo(est.id, est.activo)} className={`w-full inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-semibold transition-all ${est.activo ? "bg-green-100 text-green-700 border border-green-200" : "bg-orange-100 text-orange-700 border border-orange-300"}`}>
                                        {est.activo ? (<><FaCheck className="mr-2" /> Activo</>) : (<><FaCheck className="mr-2" /> Reactivar</>)}
                                      </button>
                                      <div className="flex gap-2">
                                        <button onClick={() => handleEdit(est)} className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 text-sm font-medium transition-all"><FaEdit /> Editar</button>
                                        <button onClick={() => handleDelete(est.id)} className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 text-sm font-medium transition-all"><FaTrash /> Desactivar</button>
                                      </div>
                                    </div>
                                  )}
                                  {!puedeEditar && (
                                    <div className="mt-2 text-xs text-slate-400 italic md:hidden flex items-center gap-1"><FaLock className="text-[10px]" /> Solo lectura</div>
                                  )}
                                </div>
                              </td>
                              {esAdmin && (
                                <td className="hidden md:table-cell px-5 py-4">
                                  {grado && (
                                    <div className="flex items-center gap-2">
                                      <div className="bg-linear-to-br from-blue-500 to-purple-600 text-white rounded w-6 h-6 flex items-center justify-center text-xs font-bold">{grado.paralelo}</div>
                              ...
                              {puedeEditar && (
                                <td className="hidden md:table-cell px-5 py-4 text-center">
                                  <button onClick={() => handleToggleActivo(est.id, est.activo)} className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold transition-all ${est.activo ? "bg-green-100 text-green-700 border border-green-200 hover:bg-green-200" : "bg-orange-100 text-orange-700 border border-orange-300 hover:bg-orange-200"}`}>
                              ...
                              {puedeEditar && (
                                <td className="hidden md:table-cell px-5 py-4">
                                  <div className="flex justify-center gap-2">
                                    <button onClick={() => handleEdit(est)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg transition-all bg-blue-50 text-blue-600 hover:bg-blue-100 text-xs font-medium"><FaEdit /> Editar</button>
                                    <button onClick={() => handleDelete(est.id)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg transition-all bg-red-50 text-red-600 hover:bg-red-100 text-xs font-medium"><FaTrash /> Desactivar</button>
                                  </div>
                                </td>
                              )}
                            </tr>
            ...
            <div className="mt-4 bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-purple-900">
                  <p className="font-semibold mb-1">{tutorDeAnioActivo.length > 0 ? `Eres tutor de ${tutorDeAnioActivo.length} grado(s) en ${anioActivo?.nombre}` : "No eres tutor de ningún grado en este año lectivo"}</p>
                  <p className="text-xs">{tutorDeAnioActivo.length > 0 ? "Puedes registrar, editar y desactivar estudiantes solo en los grados donde eres tutor. Los estudiantes desactivados conservan su historial académico y pueden reactivarse." : "Como docente sin rol de tutor, solo puedes visualizar los estudiantes de tus grados asignados."}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="bg-linear-to-r from-slate-50 to-slate-100 px-6 pt-6 pb-4 border-b border-slate-200">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-full bg-slate-200 flex items-center justify-center shrink-0"><ConfirmIcon className="text-slate-700 text-xl" /></div>
                <div className="flex-1 min-w-0">
